    ...
    Future(()async{
      ...
      controller?.initialize().then((_) {
        ...
        controller!.startImageStream(
              ...
              List<Map<String, dynamic>> boxes = await Opencv().runYolo(frame: frame);
              String? tempPrediction;
              for(var box in boxes){
                tempPrediction = tempPrediction?? "" + " " + box['class'];
              }
              await Future.delayed(Duration(milliseconds: 600));

              debugPrint("Found ${boxes.length} boxes");
              if(mounted){
                setState(() {
